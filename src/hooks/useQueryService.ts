...
export const useQueryService = (): QueryServiceHookResult => {
  ...
  const fetchQueryById = useCallback(
    async (id: string): Promise<IQuery | null> => {
  ...
  const fetchQueriesByUser = useCallback(
    async (userId: string): Promise<IQuery[]> => {
  ...
  const fetchQueriesByStatus = useCallback(
    async (status: string): Promise<IQuery[]> => {
      if (!isConnected) return [];
  ...
  const createQuery = useCallback(
    async (queryData: Partial<IQuery>): Promise<IQuery | null> => {
      if (!isConnected) return null;
  ...
  const updateQuery = useCallback(
    async (id: string, queryData: Partial<IQuery>): Promise<IQuery | null> => {
      if (!isConnected) return null;
      ...
      try {
        ...
        if (updatedQuery) {
          setQueries(prev => prev.map(query => (query._id === id ? updatedQuery : query)));
        }
