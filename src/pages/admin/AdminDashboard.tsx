...
const AdminDashboard: React.FC = () => {
  ...
  const dashboardStats = {
    ...
    recentActivities: [
      ...
      { id: 3, user: 'System', action: 'automatically closed inactive queries', time: '1 hour ago' },
      { id: 4, user: 'Mike Brown', action: 'updated query #1009 status', time: '2 hours ago' },
    ]
  };
  ...
  const toggleStatusDropdown = (e: React.MouseEvent, queryId: number) => {
    e.stopPropagation();
    setStatusDropdown(statusDropdown === queryId ? null : queryId);
  };
                            ...
                            {statusDropdown === query.id && (
                              <div className="status-dropdown">
                                {statusOptions.map((status) => (
                                  ...
                                  >
                                    {status}
                                    {query.status === status && <FiCheck className="check-icon" />}
                                  </button>
                                ))}
                              </div>
                            )}
